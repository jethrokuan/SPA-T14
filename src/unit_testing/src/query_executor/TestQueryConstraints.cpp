...
TEST_CASE ("Test Query Constraints swapping functionality") {
  ...
  qc.addToSingleVariableConstraints("y", std::unordered_set<std::string>{"4", "5", "6"});
  qc.addToPairedVariableConstraints("x", "y", {{"1", "2"}, {"3", "4"}});
  qc.addToPairedVariableConstraints("y", "x", {{"3", "1"}, {"4", "3"}});

  SingleVariableConstraints s0 = {"x", std::unordered_set<std::string>{"1", "2", "3"}};
  SingleVariableConstraints s1 = {"y", std::unordered_set<std::string>{"4", "5", "6"}};
...
TEST_CASE ("Test Query Constraints isVarInAllPossibleValues functionality") {
  ...
  qc.addToAllPossibleValues("x", std::unordered_set<std::string>{"1", "2", "3"});
  REQUIRE(qc.isVarInAllPossibleValues("x") == true);
  REQUIRE(qc.isVarInAllPossibleValues("y") == false);

  qc.addToAllPossibleValues("y", std::unordered_set<std::string>{"4", "5", "6"});

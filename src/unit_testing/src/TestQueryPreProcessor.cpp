...
TEST_CASE (
  ...
  SECTION ("Pattern test UnderScore QuoteIdent") {
    auto qp = QE::QueryPreprocessor();
    std::string input =
            "assign p;Select p such that Follows(a, b) pattern p (_, \"x\")";
    auto query = qp.getQuery(input);
    REQUIRE(*(query->declarations) ==
            std::vector<Declaration>{Declaration(
                    DesignEntity::ASSIGN, QE::Synonym::construct("p").value())});
    ...
    QE::ExpressionSpec expr = QE::QuoteIdent::construct("\"x\"").value()
    auto pat = Pattern::construct(syn, entRef, expr);
    REQUIRE(*(query->pattern) == pat);
  }
  ...
  SECTION ("Pattern test DoubleUnderscoreFactor UnderScore") {
    auto qp = QE::QueryPreprocessor();
    std::string input = "assign p;Select p such that Follows(a, b) pattern p (_\"y\"_, _)";
    REQUIRE_THROWS_AS(qp.getQuery(input), QE::PQLParseException);
  }
}
...
TEST_CASE (
    ...
    SECTION ("Pattern test QuoteIdent DoubleUnderscoreFactor") {
        auto qp = QE::QueryPreprocessor();
        std::string input =
                "assign p;Select p such that Uses(a, b) pattern p (\"x\", "
                "_\"y\"_)";
        auto query = qp.getQuery(input);
        REQUIRE(*(query->declarations) ==
                std::vector<Declaration>{Declaration(
    ...
    SECTION ("Pattern test UnderScore QuoteIdent") {
        auto qp = QE::QueryPreprocessor();
        std::string input =
                "assign p;Select p such that Uses(a, b) pattern p (_, \"x\")";
        auto query = qp.getQuery(input);
        REQUIRE(*(query->declarations) ==
                std::vector<Declaration>{Declaration(
                        DesignEntity::ASSIGN, QE::Synonym::construct("p").value())});
...
TEST_CASE ("Test Preprocess Exceptions") {
  ...
  SECTION ("Test mulitple such that query Preprocess") {
    auto qp = QE::QueryPreprocessor();
    std::string input = "assign p;Select p such that Follows(p , q) such that Modifies(p , q)";
    REQUIRE_THROWS_AS(qp.getQuery(input), QE::PQLParseException);
  }

  SECTION ("Test mulitple pattern query Preprocess") {
    auto qp = QE::QueryPreprocessor();

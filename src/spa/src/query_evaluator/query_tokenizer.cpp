...
QueryTokenizerTokens QueryTokenizer::getTokens(std::string pql_query_string) {
    ...
    if (pql_query_string.empty()) { return final_tokens; }
    ...
    auto& tokens = *(QueryTokenizer::splitString(pql_query_string, QueryTokenizer::QUERY_DELIMITER));

    // Tracking where we are in tokens
    size_t tokens_counter = 0;

    // Find the optional such_that clause and pattern clauses in the Select statement
    for (std::string t : tokens) {
        // Make sure this is the token with the select inside
        if(t.compare(0, QueryTokenizer::QUERY_SELECT.length(), QueryTokenizer::QUERY_SELECT) == 0) {
            // For each assignment token, split the tokens by space
            auto declaration_token_strings = std::vector<std::string>(tokens.begin(),tokens.begin() + tokens_counter);
            ...
            for (std::string s : declaration_token_strings) {
                ...
                s+= " ";
                auto split_declaration_tokens = QueryTokenizer::splitString(s, QueryTokenizer::SPACE_DELIMITER);
                final_tokens.declaration_tokens->insert(final_tokens.declaration_tokens->end(),split_declaration_tokens->begin(),split_declaration_tokens->end());
            }
...
void QueryTokenizer::setClauses(std::string& select_clause, QueryTokenizerTokens& qtt) {
    // Space separate all tokens to begin analysis
    auto initial_clause_tokens = QueryTokenizer::splitString(select_clause, QueryTokenizer::SPACE_DELIMITER);
    ...
    if (initial_clause_tokens->size() == 2) { return; }

    // Find such_that position if exists
    size_t such_that_pos = 0;
    for (size_t i = 2; i < initial_clause_tokens->size(); i++) {
        if ((*initial_clause_tokens)[i] == "such" && (*initial_clause_tokens)[i+1] == "that") {
            such_that_pos = i;
        }
    }
    ...
    if (such_that_pos != 0) {
        // We have a such that clause - can take from here until the end of string OR pattern
        auto first = initial_clause_tokens->begin() + such_that_pos;
        auto last = pattern_pos == 0 ? initial_clause_tokens->end() : initial_clause_tokens->begin() + pattern_pos;
    ...
    if (pattern_pos != 0) {
        ...
        auto first = initial_clause_tokens->begin() + pattern_pos;
...
std::vector<std::string>* QueryTokenizer::splitString(std::string str, const char delimiter) {
...
// This namespace definition is needed since this function needs to be part of the QE namespace
namespace QE {
    std::ostream& operator <<(std::ostream& outputStream, const QueryTokenizerTokens& qtt){
        outputStream << "\nDeclaration Tokens:\n\t";

...
Query* QueryPreprocessor::getQuery(std::string& pql_query_string) {
    ...
    QueryTokenizerTokens query_tokens = query_tokenizer.getTokens(pql_query_string);
...
void QueryPreprocessor::parseSelect(Query* query, std::vector<std::string>* select_tokens) {
    ...
    auto found_declaration = std::find_if(query->declarations->begin(),
        query->declarations->end(),
        [&](auto decl) { return decl.synonym == synonym_to_match; });
    
    query->selected_declaration = &query->declarations->at(std::distance(query->declarations->begin(), found_declaration));
}

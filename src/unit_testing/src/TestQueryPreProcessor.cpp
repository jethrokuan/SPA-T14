...
TEST_CASE("Test one assign one select query Preprocess") {
    ...
    REQUIRE(*(query->declarations) == std::vector<Declaration>{Declaration(DesignEntity::ASSIGN, "p")});
    REQUIRE(*(query->selected_declaration) == Declaration(DesignEntity::ASSIGN, "p"));
}
...
TEST_CASE("Test two assign one select query Preprocess") {
    ...
    auto query = qp.getQuery(input);
    REQUIRE(*(query->declarations) == std::vector<Declaration>{Declaration(DesignEntity::ASSIGN, "p"), Declaration(DesignEntity::STMT, "s")});
    REQUIRE(*(query->selected_declaration) == Declaration(DesignEntity::ASSIGN, "p"));
}
...
TEST_CASE("Test three assign one select query Preprocess") {
    ...
    auto query = qp.getQuery(input);
    REQUIRE(*(query->declarations) == std::vector<Declaration>{Declaration(DesignEntity::ASSIGN, "p"), Declaration(DesignEntity::STMT, "s"), Declaration(DesignEntity::ASSIGN, "q")});
    REQUIRE(*(query->selected_declaration) == Declaration(DesignEntity::ASSIGN, "p"));
}
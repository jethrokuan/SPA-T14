...
class AffectsEvaluator : public SuchThatEvaluator {
 public:
  AffectsEvaluator(std::vector<QE::Declaration>* decls, QE::RelCond* relCond,
  ...
  bool handleDoubleUnderscore() override {
    ...
    for (auto lhs_de : lhs_designentities) {
      for (auto rhs_de : rhs_designentities) {
        if (pkb->isLineAffectsLine(lhs_de, rhs_de)) {
          return true;
        }
      }
    }
  ...
  bool handleLeftBasicRightUnderscore(std::string& arg) override {
    // Affects(1, _)
    return pkb->getAffectUsesLine(arg).has_value();
  }
  bool handleRightBasicLeftUnderscore(std::string& arg) override {
    // Affects(_, 1)
    return pkb->getAffectModifiesLine(arg).has_value();
  }
